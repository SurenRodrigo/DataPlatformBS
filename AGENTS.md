...
**Note**: Service enablement (hasura, metabase, superset, dagster, nginx, db mode) is controlled via `service.yaml` at project root, not via CLI flags. The `--with-nginx` and `--no-local-db` flags have been removed in favor of `service.yaml` configuration.
