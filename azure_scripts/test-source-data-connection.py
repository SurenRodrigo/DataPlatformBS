#!/usr/bin/env python3
"""
Test script to verify Service Principal connection to Azure SQL Database (source_data)
"""
import os
import sys
...
# Get environment variables for SOURCE_DATA database
...
if not all([client_id, client_secret, tenant_id]):
    ...
    print(f"  AZURE_SOURCE_DATA_CLIENT_ID: {'✓' if client_id else '✗'}")
    print(f"  AZURE_SOURCE_DATA_CLIENT_SECRET: {'✓' if client_secret else '✗'}")
    print(f"  AZURE_SOURCE_DATA_TENANT_ID: {'✓' if tenant_id else '✗'}")
    sys.exit(1)

print("=" * 60)
print("Testing Service Principal Connection to SOURCE_DATA Database")
print("=" * 60)
...
try:
    ...
    # Test query 4: List tables in Sales schema
    print("Test 4: Listing tables in Sales schema...")
    try:
        cursor.execute("""
    ...
    print("1. Verify the source_data database exists and is online")
    print("2. Check that the service principal has the correct permissions on source_data")
    print("3. Verify the service principal credentials are correct")
    print("4. Check if the database is paused (Azure SQL can be paused)")
    print("5. Use Azure Portal to check database status")
    sys.exit(1)
except Exception as e:
